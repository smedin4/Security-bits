...
If some `ExportContainerWindowDelta` activities fail with `DF-Executor-InvalidOutputColumns` ("at Sink 'DeltaSink': The result has 0 output columns. Please ensure at least one column is mapped"), the affected tables had **no `PT5M` files in the processed hour**. The export source uses schema drift, so an empty hour produces a stream with zero columns; the flow avoids an empty sink by always deriving an `IngestedUtc` column, and the `df_seed_table_schema` step that runs first stamps the full declared schema. A quiet hour therefore writes an empty (0-row) Delta commit instead of failing. If you still hit this error, redeploy the latest template. `IngestedUtc` is the processing time, not the event time — use the source `TimeGenerated` column for event-time queries.
...
Every federated table also carries its **full declared schema**, regardless of data. Before each export, `df_seed_table_schema` stamps every declared column (correctly typed) from `_raw/workspace-tables.json` as a 0-row Delta commit, so `TimeGenerated` and all declared columns are present even on hours with no data, and new workspace columns appear on the next run (add-only; `mergeSchema` never drops). A column that is genuinely absent will not appear until it is **declared** in the workspace and the schema refresh picks it up.

### A table fails with a Delta schema/type merge conflict

Native typing reads each table's **declared** column types (which are stable across runs), so the per-run schema does not drift and merge conflicts do not arise in normal operation. A conflict almost always means a table's existing Delta folder holds a **different type** for some column than the current declared schema — for example a column's declared type changed, or the folder was created by an older all-string build of this template. Switching a column between string and a native type is an incompatible Delta change. **Fix:** delete that table's folder (including its `_delta_log/`) so the next run rebuilds it cleanly from the seed, or point `targetFileSystem` at a fresh file system and re-point federation to it. A clean run against an empty target needs no resets.

A conflict naming a single field with a `StructType` versus `StringType` (for example `Failed to merge fields 'LoggedOnUsers' ... StructType(...) and StringType`) or an error about invalid characters in nested column names is a **`dynamic` column** that was not stringified deterministically. The export now uses the workspace schema map to identify declared `dynamic` columns and stringify them by name (`dynamicColsCsv`) before writing. If you see this, redeploy the latest template and **delete the affected table's folder once** (for example `deviceinfo/` or `devicenetworkevents/`, including `_delta_log/`) so it rebuilds with those columns as JSON strings — query them with `parse_json()`.
...
If the **export is skipped entirely**, confirm the managed identity has **Reader on the Log Analytics workspace** — the export refreshes the schema first and stops if that refresh fails.
...
1. In Data Factory Studio, open **Author → Data flows → `df_seed_table_schema`** (or `df_json_to_delta_native`).
