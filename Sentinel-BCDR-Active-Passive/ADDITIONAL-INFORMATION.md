...
A conflict naming a single field with a `StructType` versus `StringType` (for example `Failed to merge fields 'LoggedOnUsers' ... StructType(...) and StringType`) is a **`dynamic` column** whose JSON value is an object/array in some hours and empty in others, so the JSON reader infers a struct one hour and a string the next. The data flows serialize every complex column to string (`StringifyComplex`) before writing, which keeps the type stable. If you see this, redeploy the latest template and **delete the affected table's folder once** (for example `deviceinfo/`, including `_delta_log/`) so it rebuilds with the column as a JSON string — query it with `parse_json()`.
