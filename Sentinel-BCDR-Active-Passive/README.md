...
- **JSON Lines is re-encoded to Parquet.** Each line is parsed into a row and the values are written column by column with Snappy. Because the flow is schema-drift with no fixed projection (so one flow handles every table's different and unpredictable shape), drifted columns are read **as strings** (`inferDriftedColumnTypes: false`). This is deliberate: the same Azure Monitor column can infer as different types across hours (number vs boolean, integer vs long), which would otherwise break the Delta schema merge. Cast at query time (`toint()`, `tolong()`, `tobool()`); the `IngestedUtc` column added by the flow stays a timestamp. Row-oriented text becomes columnar binary.
...
If a table fails with `Failed to merge incompatible data types` (for example, "Failed to merge fields 'ResultType' and 'ResultType' ... IntegerType and BooleanType", or "'Total' ... IntegerType and LongType"), a column's **inferred type changed between runs**. Azure Monitor JSON is schema-on-read, so a value like `ResultType` can look like a number in one hour (`0`) and a boolean in another (`true`), and `Total` can be a small integer one hour and a long the next. When the Delta sink merges this hour's inferred schema with the existing table, Delta refuses to combine incompatible types. The template avoids this by reading every drifted column **as a string** (`inferDriftedColumnTypes: false` on the source), so a column never changes type between runs and strings always merge. Cast at query time instead (`toint()`, `tolong()`, `tobool()`). If you are upgrading from a build that inferred types, the existing tables were written with typed columns and will still conflict when string data is appended; **reset the affected tables once** — either point `targetFileSystem` at a fresh file system (for example `sentinel-bcdr2`) and re-point federation to it, or delete the existing `<table>/` folders (including `_delta_log/`) so the schedule rebuilds them as all-string.
